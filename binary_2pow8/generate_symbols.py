...
from binary_2pow8.config import MIN_INT, MAX_INT, field

from icecream import ic
from binary_2pow8.operations_bin8 import inner_product_bytes_bin8, print_ints
...
def generate_symbols_random_bin8(data_fields:int, gen_size:int) -> list:
...
def check_orth_fixed(generation:list[bytearray]) -> bool:
    ...
    for i, packet in enumerate(new_gen):
        for j, p in enumerate(new_gen):
            prod = inner_product_bytes_bin8(packet, p)
            if prod != 0:
...
def check_orth_bin8(generation:list[bytearray], log_dir: Path | None = None) -> bool:
